...
클라이언트 사이드 취약점(XSS 등)이 주로 사용자 브라우저나 세션을 타겟으로 한다면, 서버사이드 취약점은 **데이터와 인프라의 핵심을 직접 타격**하므로 포트스위거 아카데미에서도 가장 중요하고 난이도 높게 다루는 파트 중 하나예요. PortSwigger 실습하시면서 어떤 세부 챕터에서 막히시나요?
...
질문하신 것처럼 **`/admin` 같은 관리자 페이지 URL이 숨겨져 있지 않고 노출되거나 유추 가능해서 발생하는 문제**도 접근 제어 취약점의 대표적인 유형(예측 가능한 리소스 위치 및 권한 우회)이 맞아요.
